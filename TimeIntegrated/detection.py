...
#Kamiokande-II
def dN_Kamiokande_Garching(E_det,cos_theta,E_0_e,L_e,alpha_e,E_0_x,L_x,alpha_x,Pee,mass_ord,eff_typ="intrinsic"):
	...
	dN_IB = np.where(Enu_detc_IB <=0,0.0,eff(E_det,'K',typ=eff_typ,E_min=4.5)*dEnu_dEe(E_det,cos_theta)*Np*cross_CC(Enu_detc_IB)* antinu_e_flux_Garching(Enu_detc_IB,E_0_e,L_e,alpha_e,E_0_x,L_x,alpha_x,Pee,mass_ord))
	return dN_IB #MeV⁻¹
#dN_Kamiokande_Garching_vec=np.vectorize(dN_Kamiokande_Garching)

##### IMB ##########
def dN_IMB_Garching(E_det,cos_theta,E_0_e,L_e,alpha_e,E_0_x,L_x,alpha_x,Pee,mass_ord,eff_typ="intrinsic"):
	#http://arxiv.org/abs/astro-ph/0107260 - Lamb & Loredo
	#Mass and number of targets
	#https://arxiv.org/pdf/1606.00665.pdf
	#https://www.researchgate.net/publication/23826760_A_study_of_atmospheric_neutrinos_with_the_IMB_detector
    ...
    #Inverse beta decay event rate
	Enu_detc_IB=Enu_IB(E_det,cos_theta)
	dN_IB = np.where(Enu_detc_IB <=0,0.0,eff(E_det,'I',typ=eff_typ,E_min=15)*dEnu_dEe(E_det,cos_theta)*Np*cross_CC(Enu_detc_IB)* antinu_e_flux_Garching(Enu_detc_IB,E_0_e,L_e,alpha_e,E_0_x,L_x,alpha_x,Pee,mass_ord))
	return dN_IB #MeV⁻¹
#dN_IMB_Garching_vec=np.vectorize(dN_IMB_Garching)

##### Baksan ##########
def dN_Baksan_Garching(E_det,cos_theta,E_0_e,L_e,alpha_e,E_0_x,L_x,alpha_x,Pee,mass_ord,eff_typ="intrinsic"):
	#http://arxiv.org/abs/astro-ph/0107260 - Lamb & Loredo
    ...
    #Inverse beta decay event rate
	Enu_detc_IB=Enu_IB(E_det,cos_theta)
	dN_IB = np.where(Enu_detc_IB <=0,0.0,eff(E_det,'B',typ=eff_typ,E_min=10)*dEnu_dEe(E_det,cos_theta)*Np*cross_CC(Enu_detc_IB)* antinu_e_flux_Garching(Enu_detc_IB,E_0_e,L_e,alpha_e,E_0_x,L_x,alpha_x,Pee,mass_ord))
	return dN_IB #MeV⁻¹
...
def dN_Kamiokande_gaussian_Garching_vec(E,Epos, delta_Epos,cos_theta,E_0_e,L_e,alpha_e,E_0_x,L_x,alpha_x,Pee,mass_ord):
  gauss_R = dN_Kamiokande_Garching(E,cos_theta,E_0_e,L_e,alpha_e,E_0_x,L_x,alpha_x,Pee,mass_ord,"intrinsic")*Gaussian_E(E, Epos, delta_Epos)
  return gauss_R
#dN_Kamiokande_gaussian_Garching_vec=np.vectorize(dN_Kamiokande_gaussian_Garching)

def dN_IMB_gaussian_Garching_vec(E,Epos, delta_Epos,cos_theta,E_0_e,L_e,alpha_e,E_0_x,L_x,alpha_x,Pee,mass_ord):
  gauss_R = dN_IMB_Garching(E,cos_theta,E_0_e,L_e,alpha_e,E_0_x,L_x,alpha_x,Pee,mass_ord,"intrinsic")*Gaussian_E(E, Epos, delta_Epos)
  return gauss_R
#dN_IMB_gaussian_Garching_vec=np.vectorize(dN_IMB_gaussian_Garching)

def dN_Baksan_gaussian_Garching_vec(E,Epos, delta_Epos,cos_theta,E_0_e,L_e,alpha_e,E_0_x,L_x,alpha_x,Pee,mass_ord):
  gauss_R = dN_Baksan_Garching(E,cos_theta,E_0_e,L_e,alpha_e,E_0_x,L_x,alpha_x,Pee,mass_ord,"intrinsic")*Gaussian_E(E, Epos, delta_Epos)
  return gauss_R
#dN_Baksan_gaussian_Garching_vec=np.vectorize(dN_Baksan_gaussian_Garching)

def dN_Kamiokande_exp_Garching(Epos,delta_Epos,cos_theta,E_0_e,L_e,alpha_e,E_0_x,L_x,alpha_x,Pee,mass_ord) :
	E_max=Epos+5*delta_Epos
	E_min=Epos-5*delta_Epos
	if E_min<0.52:
		E_min=0.52
	E_vec=np.linspace(E_min,E_max,101)
	dN = integrate.simps(dN_Kamiokande_gaussian_Garching_vec(E_vec[:,np.newaxis],Epos, delta_Epos,cos_theta,E_0_e,L_e,alpha_e,E_0_x,L_x,alpha_x,Pee,mass_ord), E_vec, axis=0)
	return dN
dN_Kamiokande_exp_Garching_vec=np.vectorize(dN_Kamiokande_exp_Garching)

def dN_IMB_exp_Garching(Epos,delta_Epos,cos_theta,E_0_e,L_e,alpha_e,E_0_x,L_x,alpha_x,Pee,mass_ord) :
	E_max=Epos+5*delta_Epos
	E_min=Epos-5*delta_Epos
	if E_min<0.52:
		E_min=0.52
	E_vec=np.linspace(E_min,E_max,101)
	dN = integrate.simps(dN_IMB_gaussian_Garching_vec(E_vec[:,np.newaxis],Epos, delta_Epos,cos_theta,E_0_e,L_e,alpha_e,E_0_x,L_x,alpha_x,Pee,mass_ord), E_vec, axis=0)
	return dN
dN_IMB_exp_Garching_vec=np.vectorize(dN_IMB_exp_Garching)

def dN_Baksan_exp_Garching(Epos,delta_Epos,cos_theta,E_0_e,L_e,alpha_e,E_0_x,L_x,alpha_x,Pee,mass_ord) :
	E_max=Epos+5*delta_Epos
	E_min=Epos-5*delta_Epos
	if E_min<0.52:
		E_min=0.52
	E_vec=np.linspace(E_min,E_max,101)
	dN = integrate.simps(dN_Baksan_gaussian_Garching_vec(E_vec[:,np.newaxis],Epos, delta_Epos,cos_theta,E_0_e,L_e,alpha_e,E_0_x,L_x,alpha_x,Pee,mass_ord), E_vec, axis=0)
	return dN
dN_Baksan_exp_Garching_vec=np.vectorize(dN_Baksan_exp_Garching)
